...
def convert():
    ...
    global stringFile
    global numLines
    global textFileArray
    global scrollDir
    global letterToPrint
    ...
    #        pygame.quit()
        print('Before textPos')
        if event.type == pygame.MOUSEBUTTONDOWN:
            print('Trying to print')
            if((textPos==0 or textPos==-1) and event.button == 5):
                print("You are at the beginning of the document!")
                #PLAY AUDIO HERE
            elif event.button == 4: #positive direction sequence
                ...
                textPos = textPos + 1
            elif event.button == 5: #negative direction sequence
                if((ord(textFileArray[textPos])>=65 and ord(textFileArray[textPos])<=122) and (ord(textFileArray[textPos-1])>=48 and ord(textFileArray[textPos-1])<=58)):
                    print("numInd")
                    print('o'+"\t"+'x'+"\n"+'o'+"\t"+'x'+"\n"+'x'+"\t"+'x')
                    #GPIO.output(tl, GPIO.LOW)
